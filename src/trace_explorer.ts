...
import { WebGLScene, getShader, formatTimeSpan } from './utils';
...
function getColorFromString(s) {
  ...
  const hue = (hash % 256) / 256;
  hash = Math.floor(hash / 256);
  const lightness = 0.3 + (0.4 * (hash % 256)) / 256;
  hash = Math.floor(hash / 256);
  const saturation = 0.3 + (0.4 * (hash % 256)) / 256;
...
function CreateStreamVisualizer(rectangles, uniforms) {
  ...
  const rect_ids = new Float32Array(n_rects);
  for (let i = 0; i < n_rects; i++) rect_ids[i] = i + 1;
  ...
  geo.setAttribute(
    'times',
    new THREE.InstancedBufferAttribute(new Float32Array(rectangles.times), 2)
  );
  geo.setAttribute(
    'level',
    new THREE.InstancedBufferAttribute(new Uint16Array(rectangles.level), 1)
  );
  geo.setAttribute(
    'name_id',
    new THREE.InstancedBufferAttribute(new Uint16Array(rectangles.name_id), 1)
  );
...
export class TraceViewer extends WebGLScene {
  ...
  onResize() {
...

    {
      ...
      this.buffer_texture = new THREE.WebGLRenderTarget(w, h, {
  ...
  onMouseWheel(event) {
    ...
    if (t0 < 0) t0 = 0;
    if (t1 > this.data.max_time) t1 = this.data.max_time;
    this.camera.left = t0;
  ...
  onMouseMove(event) {
    ...
    {
      if (rect.width - x < 300) {
        const sx = event.clientX - 20 + 'px';
        this.tooltip.style.right = sx;
      } else {
        const sx = event.clientX + 20 + 'px';
        this.tooltip.style.left = sx;
      }
      const sy = event.clientY + 20 + 'px';
