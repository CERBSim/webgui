...
function toString(x) {
  if (Math.abs(x) < 1e-15) return (0.0).toPrecision(2);
...
class PlaneGridWithLabels extends THREE.Group {
  ...
  updateLabelPositions(canvas, camera, matrix, visible) {
    ...
    for (const { el, p } of this.labels) {
      ...
      if (
        visible &&
        (y < rect.top || y > rect.bottom || x > rect.right || x < rect.left)
      )
...
function roundFloatTo125(x) {
  ...
  if (x < 1.5) return scale;

  if (x < 3.5) return 2 * scale;

  if (x < 7.5) return 5 * scale;
...
export function Grid3D(parent, bounding_sphere) {
  ...
  const n = 7;
  const h = roundFloatTo125((2 * radius) / n);
  ...
  const p = new THREE.Vector3();
  p.copy(center)
    .addScaledVector(vx, -0.5 * n)
    .addScaledVector(vy, -0.5 * n)
    .addScaledVector(vz, -0.5 * n);
  ...
  for (const p of planes) group.add(p);
